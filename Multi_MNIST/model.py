...
"""https://github.com/AvivNavon/pareto-hypernetworks/blob/master/experiments/multimnist/models.py"""
...
class LeNetHyper(nn.Module):
    ...
    def __init__(self, kernel_size, ray_hidden_dim=100, out_dim=10,
                 target_hidden_dim=50, n_kernels=10, n_conv_layers=2, n_hidden=1, n_tasks=2, drop=0.05):
        super().__init__()
    ...
    def forward(self, ray):
        ...
        for i in layer_types:
            ...
            for j in range(n_layers):
                if i != "conv":
                    out_dict[f"{i}{j}.weights"] = self.dropout(getattr(self, f"{i}_{j}_weights")(features))
                    out_dict[f"{i}{j}.bias"] = self.dropout(getattr(self, f"{i}_{j}_bias")(features).flatten())
                else:
                    out_dict[f"{i}{j}.weights"] = self.dropout(getattr(self, f"{i}_{j}_weights")(features))
                    out_dict[f"{i}{j}.bias"] = self.dropout(getattr(self, f"{i}_{j}_bias")(features).flatten())
        return out_dict
